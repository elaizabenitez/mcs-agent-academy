...
1. We'll ask another question for suggestions of interview questions to ask based on the evaluation criteria of a job role, and provide what the potential answers are. Enter the question below.
    ...
    The returned response firsts lists the evaluation criteria for the Power Platform Developer role (defined by the Dataverse Job Role row). Each criterion includes a percentage weight. After the evaluation criteria, questions are listed for each criterion followed by a `Model Answer`. Notice how the answer is in _first person_, where it's outlining the expected answers from the candidate to the question asked.
