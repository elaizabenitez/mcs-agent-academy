...
>   & access → Feedback**, and per-response 👍/👎 appear as **Like / Dislike** in the
>   **Preview** tab (selecting **Dislike** opens a "What went wrong?" comment panel).
> - **Analytics moved.** Reactions are now shown on the **Monitor** tab (Performance →
>   **Reactions** card / **Total reactions**), replacing the classic Analytics →
>   Satisfaction → Reactions page.
> - **⚠️ The custom-Adaptive-Card half (Labs 11.3 and 11.4) is BLOCKED.** The new
>   experience has **no Topics authoring canvas** — there is no *End of Conversation*
>   system topic, no *Ask with adaptive card* / *Condition* / *Set a variable value* /
>   *Go to another topic* / *Log a custom telemetry event* nodes, and no *CSAT Question*
>   system topic to hook into. Those exercises cannot be built as written. They are
>   preserved below (collapsed) for reference, with a forward-looking note on the new
>   model.
...
1. How to use built-in thumbs up/down (**Like / Dislike**) reactions to collect feedback from users
1. How to analyze feedback data on the **Monitor** tab (formerly the Analytics dashboard)
1. How to create custom feedback collection using Adaptive Cards *(conceptual in the new experience — the hands-on Topics build is not available; see Lab 11.3)*
1. How to implement conditional feedback flows based on CSAT ratings *(conceptual — requires the classic Topics canvas)*
1. How to log custom telemetry events to Azure Application Insights for advanced feedback tracking *(conceptual — requires the classic Topics canvas)*
...
- You can disable user feedback reactions in case you don't want to gather this data (or during testing phases). In the new experience, open the agent's **Settings** (select **More options** ⋯ in the agent toolbar → **Settings**), then go to the **Safety & access** tab. Under the **Feedback** heading there is a **User feedback** toggle described as *"Show thumbs up/down on each response so users can rate quality."* By default it's **On**.

    ![User feedback toggle on the Safety & access settings tab](./assets/validation/m11-settings-user-feedback.png)

    > [!NOTE]
    > In the classic experience this lived in **Generative AI Settings → User feedback**
    > as a *Collect user reactions to agent messages* toggle, alongside a free-text
    > **disclaimer** field. In the new 4-tab Settings dialog the toggle has moved to
    > **Safety & access → Feedback**, and the separate disclaimer field is no longer
...
For the built-in interactions to be captured on the **Monitor** tab of the agent, the agent must be **published**. Ensure the **Interview Agent** is published. (You can still exercise the feedback UI without publishing by using the **Preview** tab, as shown in Lab 11.1.)

### 11.1 User feedback through built-in interactions

In the new experience you can exercise built-in feedback in two places: the **Preview** tab while authoring, and **Microsoft Teams** once the agent is published. The Preview tab is the quickest way to see the feedback flow.

#### Try it in the Preview tab

1. Open the **Interview Agent** and select the **Preview** tab. Select **New chat**, then ask the agent a question (any question will do).

1. On an agent response, you'll see a **Like** (👍) and a **Dislike** (👎) button beneath the message. Select **Dislike** to provide negative feedback.

1. A **"What went wrong?"** panel opens with a comment box. Enter a written comment, then select **Submit**. (Selecting **Like** records positive feedback; the comment panel is primarily surfaced for **Dislike**.)

    ![Built-in Dislike opens a What went wrong? comment panel in Preview](./assets/validation/m11-preview-dislike-comment.png)

    > [!NOTE]
    > In the classic experience these were **thumbs up / thumbs down** icons. In the
    > new experience they're labeled **Like / Dislike**, and selecting **Dislike**
    > opens a *"What went wrong?"* panel (with an optional checkbox to share the prompt,
    > generated response, and log files). The feedback-with-comment behavior is the same.

#### Try it in Microsoft Teams (published agent)

1. Ensure the **Interview Agent** is **published** (select **Publish** in the agent toolbar), then load it in **Microsoft Teams**.

1. In a response, select **Like** to provide positive feedback with a comment, or **Dislike** to provide negative feedback with a comment. The following are sample comments you can use:
...
We're now going to review the feedback submitted. In the new experience this lives on the **Monitor** tab (the classic **Analytics → Satisfaction → Reactions** page).

> [!NOTE]
> The submitted reactions and written feedback may take some time to show up so if you're not seeing it immediately, check it occasionally.

1. Open the **Interview Agent** and select the **Monitor** tab. Under the **Performance** section you'll find a **Total reactions** tile (in the Overview/Engagement summary) and a dedicated **Reactions** card. The **Reactions** card shows the count of 👍 vs 👎 for the selected period; until users react it reads *"No reactions yet."*

    ![Reactions card on the Monitor tab](./assets/validation/m11-monitor-reactions.png)

    > [!NOTE]
    > The classic lab navigated to the **Analytics** tab → **Satisfaction** section →
    > **Reactions** → **See details** to view thumbs counts and written feedback. In the
    > new experience the **Analytics** tab is replaced by the **Monitor** tab, where the
    > **Reactions** card (and **Total reactions** tile) surface the same satisfaction
    > signal. Detailed per-comment review of reactions still flows to Dataverse
    > conversation transcripts as before.

### 11.3 Build adaptive card to collect custom feedback

<!-- ⚠️ BLOCKED: Labs 11.3 and 11.4 are not buildable in the new Copilot Studio experience.
     Original: Create a custom Topic with an "Ask with adaptive card" node, modify the
     "End of Conversation" system topic (Condition / Set a variable value / Go to another
     topic nodes) keyed off the "CSAT Question" system topic, and add a "Log a custom
     telemetry event" node for Application Insights.
     Reason: The new experience has NO Topics authoring canvas. There are no system topics
     (End of Conversation, CSAT Question), no custom-topic creation ("Add a topic → From
     blank"), and none of the topic nodes the lab uses. The agent Build surface is
     Instructions + Skills + Tools + Knowledge + Connected agents under autonomous
     orchestration — there is no place to author this conditional CSAT flow.
     Alternative: None that reproduces the exact System-topic CSAT-trigger flow. A
     forward-looking option is to send an Adaptive Card from a Workflow tool, but the new
     model has no end-of-conversation CSAT survey trigger to branch on. Documented below;
     original steps preserved in a collapsed block for reference. -->

> [!WARNING]
> **Labs 11.3 and 11.4 cannot be completed in the new Copilot Studio experience.**
...
>   *Set a variable value*, *Go to another topic*, or *Log a custom telemetry event*.
>
> The new experience replaces topic authoring with **Instructions + Skills + Tools**
> under autonomous (generative) orchestration. Per-response feedback is fully covered by
> the built-in **Like / Dislike** mechanism you used in Lab 11.1, and surfaced on the
> **Monitor** tab in Lab 11.2.
>
> **Forward-looking alternative (not a 1:1 replacement):** If you need a custom feedback
> card, you can build a **Workflow** that posts an Adaptive Card and add it to the agent
> via **Tools → Add a tool → Workflows**, then instruct the agent (in **Instructions**)
> to invoke it after answering. However, the new experience has **no end-of-conversation
> CSAT survey trigger** to branch on (the classic *CSAT Question* system topic is gone),
> so the specific "only when the user rates 1–2 stars" routing in this exercise has no
> direct equivalent. Telemetry (Lab 11.4) would likewise move into that Workflow rather
> than a *Log a custom telemetry event* topic node.
>
> The original classic steps are preserved below for reference. **They will not work in
> the new experience** — they require switching back to the classic experience.

:::: details 📦 Original classic steps for Labs 11.3 and 11.4 (require the classic experience — NOT buildable in the new experience)
...
You learned the concepts behind collecting feedback using an adaptive card and logging telemetry to Azure Application Insights. **Note:** in the new Copilot Studio experience the hands-on build for this (Labs 11.3–11.4) is not available because the Topics authoring canvas has been removed — see the warning in Lab 11.3 for the forward-looking Workflow-based alternative.
